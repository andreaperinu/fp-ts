...
declare const compose: <E, A>(la: [A, E]) => <B>(ab: [B, A]) => [B, E]
declare const duplicate: <E, A>(ma: [A, E]) => [[A, E], E]
declare const extend: <E, A, B>(f: (fa: [A, E]) => B) => (ma: [A, E]) => [B, E]
declare const foldMap: <M>(M: Monoid<M>) => <A>(f: (a: A) => M) => <E>(fa: [A, E]) => M
declare const map: <A, B>(f: (a: A) => B) => <E>(fa: [A, E]) => [B, E]
declare const mapLeft: <E, G>(f: (e: E) => G) => <A>(fa: [A, E]) => [A, G]
declare const reduce: <A, B>(b: B, f: (b: B, a: A) => B) => <E>(fa: [A, E]) => B
declare const reduceRight: <A, B>(b: B, f: (a: A, b: B) => B) => <E>(fa: [A, E]) => B
export {
  /**
   * @since 2.0.0
   */
